function [ obj, dist ] = shiftbnd( obj, dist )
...
%  Output
%    obj    :  shifted polygon
%    dist   :  actually displacement values for each polygon

if numel( obj ) ~= 1
  dist = zeros( 1, 0 );
  for it = 1 : numel( obj )
    obj( it ) = shiftbnd( obj( it ), dist );
  end
else
  ...
  lambda = ( repmat( ( x .* ny  - y .* nx  )', numel( x ), 1 ) -  ...
                     ( x  * ny' - y  * nx' ) ) ./ in;
